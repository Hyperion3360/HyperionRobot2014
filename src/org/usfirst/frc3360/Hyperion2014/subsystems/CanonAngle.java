...
public class CanonAngle extends Subsystem {
    ...
    double throttleCorrection;
    double maxThrottleValue;
    ...
    public void HandleManualMode(){
        throttleValue = Robot.oi.getCoPilotJoystick().getRawAxis(2) + throttleCorrection;
   
        if (throttleValue > 1){
            throttleValue = 1;
        }
        
        requestedAngleValue = (maxCanonAngleValue + minCanonAngleValue)/throttleValue;
        
         if (requestedAngleValue > anglePot.getVoltage() && anglePot.getVoltage() < minCanonAngleValue){
